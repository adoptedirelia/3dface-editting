...
def generate_images(
    ...
    # Generate images.
    for seed_idx, seed in enumerate([0]):
        print('Generating image for seed %d (%d/%d) ...' % (seed, seed_idx, len([0])))
        ...
        PIL.Image.fromarray(img[0].cpu().numpy(), 'RGB').save(f'{outdir}/res.png')
