...
def gen_interp_video(G, mp4: str,w_frames=60*4, kind='cubic', grid_dims=(1,1), num_keyframes=None, wraps=2, psi=1, truncation_cutoff=14, cfg='FFHQ', image_mode='image', gen_shapes=False, device=torch.device('cuda'), **video_kwargs):
    grid_w = grid_dims[0]
    grid_h = grid_dims[1]
        ...
        for yi in range(grid_h):
            for xi in range(grid_w):
                ...
                if entangle == 'conditioning':
                    ...
                    img = G.synthesis(ws=ws, c=c_forward, noise_mode='const')[image_mode][0]
...
def generate_images(
    ...
    G.rendering_kwargs['depth_resolution'] = int(48 * sampling_multiplier)
    G.rendering_kwargs['depth_resolution_importance'] = int(48* sampling_multiplier)
    ...
    gen_interp_video(G=G, mp4=output, bitrate='10M', grid_dims=grid, num_keyframes=num_keyframes, w_frames=w_frames, psi=truncation_psi, truncation_cutoff=truncation_cutoff, cfg=cfg, image_mode=image_mode, gen_shapes=shapes)
#----------------------------------------------------------------------------
